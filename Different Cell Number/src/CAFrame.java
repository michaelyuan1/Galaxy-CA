...
public class CAFrame extends Frame{
	...
	g.drawString("outer Radius: " + String.valueOf(VCA.ringRadius(VCA.numrings() - 1)), 10, 64);
	...
	for (int i = VCA.numrings()-1; i>=0; i-=ring_drawing_step){
	    cell_angle = 360.0/VCA.numcells(i);
	    ...
	    for(int j=0; j<VCA.numcells(i); j++){
		
		if (firsttime == false){
		    curcolor = CAFrame.ACTIVE_COLOR(0.1);
		}
		if (draw_grid == true){
		    drawCellOutlines(CAscalefactor*ring_radius_under, CAscalefactor*VCA.ringRadius(i), angular_extent_of_cell(cell_angle, i, j), curcolor, g);
		}
		...
		if (state != CACell.INACTIVE){
		    ...
		    fillCell(CAscalefactor*ring_radius_under, CAscalefactor*VCA.ringRadius(i), angular_extent_of_cell(cell_angle, i, j), curcolor, g);
		} 
	...
	for (int i=theta[0]; i<=theta[1]; i++){
	    ...
	    xpointArray[counter] = (int) (cosTable[i]*inner+originx);
	    ypointArray[counter] = (int) (-sinTable[i]*inner+originy);
		i=a;
	    counter++;
	}
	for (int i=theta[1]; i >= theta[0]; i--){
    ...
    private int[] angular_extent_of_cell(double cell_angle, int ring, int number){
	double tmp;
	int res[] = new int[2];
